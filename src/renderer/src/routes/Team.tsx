...
export default function Team() {
  const {
    teamFilters, toggleTeamStatusFilter, toggleTeamPriorityFilter, clearTeamFilters,
    selectedTeamTaskId, teamDetailPanelWidth, setTeamDetailPanelWidth,
    teamSearchQuery, setTeamSearch, openCreateTeamForm
  } = useUIStore()
  const { data: tasks, isLoading } = useTeamTasks(teamFilters)
  ...
  }, [setTeamDetailPanelWidth])

  const onHandleMouseDown = (e: React.MouseEvent) => {
    e.preventDefault()
    isResizing.current = true
    startX.current = e.clientX
    startW.current = teamDetailPanelWidth
    document.body.style.cursor = 'col-resize'
    document.body.style.userSelect = 'none'
  }

  const hasFilters = !!(
    teamFilters.status?.length ||
    teamFilters.priority?.length ||
    teamFilters.project_id ||
    teamFilters.search
  )
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="px-4 py-2.5 bg-slate-850 border-b border-slate-700 flex items-center gap-2 flex-wrap">
          ...
          {STATUSES.map((s) => (
            <button
              key={s}
              onClick={() => toggleTeamStatusFilter(s)}
              className={cn(
                'px-2 py-0.5 rounded text-xs transition-colors',
                teamFilters.status?.includes(s)
          ...
          {PRIORITIES.map((p) => (
            <button
              key={p}
              onClick={() => toggleTeamPriorityFilter(p)}
              className={cn(
                'px-2 py-0.5 rounded text-xs transition-colors',
                teamFilters.priority?.includes(p)
                  ? 'bg-indigo-600 text-white'
                  : 'bg-slate-700 text-slate-400 hover:text-slate-200 hover:bg-slate-600'
              )}
            >
      ...
      {selectedTeamTaskId && (
        <>
          ...
          <div style={{ width: teamDetailPanelWidth }} className="flex-shrink-0 overflow-hidden h-full">
            <TeamTaskDetail />
          </div>
        </>
      )}
