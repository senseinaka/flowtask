// ═══════════════════════════════════════════════════════════════════════════
// Finanzas Personales — Fase 5: Bloqueo por PIN
// ═══════════════════════════════════════════════════════════════════════════
//
...
const store = new ConfigStore('finance-security')
...
export function getFinanceSecurityStatus(): FinanceSecurityStatus {
...
export function setFinancePin(pin: string): FinanceSecurityStatus {
  const salt = randomBytes(16).toString('hex')
  store.set(KEY_SALT, salt)
  store.set(KEY_HASH, hashPin(pin, salt))
  return getFinanceSecurityStatus()
}

/** Si no hay PIN configurado, cualquier intento "pasa" (no hay nada que bloquear). */
export function verifyFinancePin(pin: string): boolean {
...
export function disableFinancePin(currentPin: string): boolean {
  if (!getFinanceSecurityStatus().enabled) return true
  if (!verifyFinancePin(currentPin)) return false
...
export function changeFinancePin(currentPin: string, newPin: string): boolean {
  if (!verifyFinancePin(currentPin)) return false
  setFinancePin(newPin)
  return true
}
