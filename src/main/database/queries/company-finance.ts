...
export function listCompanyFinanceAccounts(): FinanceAccount[] {
  return getDb().prepare(`
    SELECT * FROM company_finance_accounts ORDER BY is_default DESC, name ASC
  `).all() as FinanceAccount[]
}

export function createCompanyFinanceAccount(data: CreateFinanceAccountInput): FinanceAccount {
  const db  = getDb()
  const id  = randomUUID()
  const now = Date.now()
  db.prepare(`
  ...
  return db.prepare('SELECT * FROM company_finance_accounts WHERE id = ?').get(id) as FinanceAccount
}

export function updateCompanyFinanceAccount(
  ...
  db.prepare(`UPDATE company_finance_accounts SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  return db.prepare('SELECT * FROM company_finance_accounts WHERE id = ?').get(id) as FinanceAccount
}

export function deleteCompanyFinanceAccount(id: string): void {
  getDb().prepare('DELETE FROM company_finance_accounts WHERE id = ?').run(id)
}

// ── Categories ────────────────────────────────────────────────────────────────

export function listCompanyFinanceCategories(): FinanceCategory[] {
  return getDb().prepare(`
    SELECT * FROM company_finance_categories ORDER BY is_default DESC, name ASC
  `).all() as FinanceCategory[]
}

export function createCompanyFinanceCategory(data: CreateFinanceCategoryInput): FinanceCategory {
  const db  = getDb()
  const id  = randomUUID()
  const now = Date.now()
  db.prepare(`
  ...
  return db.prepare('SELECT * FROM company_finance_categories WHERE id = ?').get(id) as FinanceCategory
}

export function updateCompanyFinanceCategory(
  ...
  if (data.color !== undefined) { sets.push('color = ?'); vals.push(data.color) }
  sets.push('updated_at = ?'); vals.push(now); vals.push(id)
  db.prepare(`UPDATE company_finance_categories SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  return db.prepare('SELECT * FROM company_finance_categories WHERE id = ?').get(id) as FinanceCategory
}

export function deleteCompanyFinanceCategory(id: string): void {
  getDb().prepare('DELETE FROM company_finance_categories WHERE id = ?').run(id)
}
...
export function listCompanyFinancePaymentMethods(): FinancePaymentMethodEntity[] {
  return getDb().prepare(`
    SELECT * FROM company_finance_payment_methods ORDER BY is_default DESC, name ASC
  `).all() as FinancePaymentMethodEntity[]
}

export function createCompanyFinancePaymentMethod(data: CreateFinancePaymentMethodInput): FinancePaymentMethodEntity {
  const db  = getDb()
  const id  = randomUUID()
  const now = Date.now()
  db.prepare(`
  ...
  return db.prepare('SELECT * FROM company_finance_payment_methods WHERE id = ?').get(id) as FinancePaymentMethodEntity
}

export function updateCompanyFinancePaymentMethod(
  ...
  if (data.color !== undefined) { sets.push('color = ?'); vals.push(data.color) }
  sets.push('updated_at = ?'); vals.push(now); vals.push(id)
  db.prepare(`UPDATE company_finance_payment_methods SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  return db.prepare('SELECT * FROM company_finance_payment_methods WHERE id = ?').get(id) as FinancePaymentMethodEntity
}

export function deleteCompanyFinancePaymentMethod(id: string): void {
  getDb().prepare('DELETE FROM company_finance_payment_methods WHERE id = ?').run(id)
}
...
export function listCompanyFinanceConcepts(opts?: { activeOnly?: boolean }): FinanceConcept[] {
  const db = getDb()
...
export function getCompanyFinanceConcept(id: string): FinanceConcept | null {
  const db = getDb()
  const r = db.prepare(`${CONCEPT_SELECT} WHERE co.id = ?`).get(id) as ConceptRow | undefined
  return r ? hydrateConcept(r) : null
}

export function createCompanyFinanceConcept(data: CreateFinanceConceptInput): FinanceConcept {
  const db  = getDb()
  const id  = randomUUID()
  const now = Date.now()
  db.prepare(`
  ...
  return getCompanyFinanceConcept(id)!
}

export function updateCompanyFinanceConcept(
  ...
  sets.push('updated_at = ?'); vals.push(now); vals.push(id)
  db.prepare(`UPDATE company_finance_concepts SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  return getCompanyFinanceConcept(id)!
}

export function deleteCompanyFinanceConcept(id: string): void {
  getDb().prepare('DELETE FROM company_finance_concepts WHERE id = ?').run(id)
}
...
export function listCompanyFinanceMovements(month: number, year: number): FinanceMovement[] {
  const db = getDb()
 ...
 */
export function listUpcomingCompanyFinanceMovements(): FinanceMovement[] {
  const db = getDb()
  const rows = db.prepare(`
    ${MOVEMENT_BASE_SELECT}
...
export function getCompanyFinanceMovement(id: string): FinanceMovement | null {
  const db = getDb()
  const r = db.prepare(`${MOVEMENT_BASE_SELECT} WHERE m.id = ?`).get(id) as MovementRow | undefined
  return r ? hydrateMovement(r) : null
}

export function createCompanyFinanceMovement(data: CreateFinanceMovementInput): FinanceMovement {
  const db  = getDb()
  const id  = randomUUID()
  const now = Date.now()
  db.prepare(`
  ...
  return getCompanyFinanceMovement(id)!
}

export function updateCompanyFinanceMovement(
  ...
  sets.push('updated_at = ?'); vals.push(now); vals.push(id)
  db.prepare(`UPDATE company_finance_movements SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  return getCompanyFinanceMovement(id)!
}

/** Edición rápida desde la tabla principal: monto real, estado, fechas y notas. */
export function quickUpdateCompanyFinanceMovement(id: string, data: {
...
export function deleteCompanyFinanceMovement(id: string): void {
  getDb().prepare('DELETE FROM company_finance_movements WHERE id = ?').run(id)
}
...
export function addMovementEntry(data: CreateFinanceMovementEntryInput): FinanceMovementEntry {
  ...
  const run = db.transaction(() => {
    db.prepare(`
      INSERT INTO company_finance_movement_entries (id, movement_id, amount, entry_date, note, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, data.movement_id, data.amount, data.entry_date ?? now, data.note ?? '', now, now)
    recalcMovementFromEntries(db, data.movement_id)
  })
  run()
  return db.prepare('SELECT * FROM company_finance_movement_entries WHERE id = ?').get(id) as FinanceMovementEntry
}

export function updateMovementEntry(id: string, data: UpdateFinanceMovementEntryInput): FinanceMovementEntry {
  const db  = getDb()
  const now = Date.now()
  const existing = db.prepare('SELECT * FROM company_finance_movement_entries WHERE id = ?').get(id) as FinanceMovementEntry | undefined
  ...
  const run = db.transaction(() => {
    ...
    sets.push('updated_at = ?'); vals.push(now); vals.push(id)
    db.prepare(`UPDATE company_finance_movement_entries SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
...
export function removeMovementEntry(id: string): void {
  const db = getDb()
  const existing = db.prepare('SELECT movement_id FROM company_finance_movement_entries WHERE id = ?').get(id) as { movement_id: string } | undefined
  if (!existing) return
  const run = db.transaction(() => {
    db.prepare('DELETE FROM company_finance_movement_entries WHERE id = ?').run(id)
    recalcMovementFromEntries(db, existing.movement_id)
  })
  run()
}
...
function shouldGenerateMovement(db: ReturnType<typeof getDb>, concept: FinanceConcept, month: number): boolean {
  switch (concept.recurrence) {
    ...
    case 'one_time': {
      const already = db.prepare('SELECT 1 FROM company_finance_movements WHERE concept_id = ? LIMIT 1').get(concept.id)
      return !already
    }
    case 'monthly':
    case 'biweekly':
...
function generateMovements(month: number, year: number, estimateSource: 'default' | 'previous'): FinanceMovement[] {
  const db = getDb()
  const now = Date.now()
  const concepts = listCompanyFinanceConcepts({ activeOnly: true })
  const existing = new Set(
    (db.prepare('SELECT concept_id FROM company_finance_movements WHERE month = ? AND year = ?').all(month, year) as { concept_id: string }[])
      .map(r => r.concept_id)
  )
  ...
  if (estimateSource === 'previous') {
    const prevDate = new Date(year, month - 2, 1)
    const prevMovements = listCompanyFinanceMovements(prevDate.getMonth() + 1, prevDate.getFullYear())
...
export function getCompanyFinanceMonthSummary(month: number, year: number): FinanceMonthSummary {
  ...
  const prevMovements = listCompanyFinanceMovements(prevMonth, prevYear)
...
export function getCompanyFinanceMonthInsight(month: number, year: number): FinanceMonthInsight | null {
  return (getDb()
    .prepare('SELECT * FROM company_finance_month_insights WHERE month = ? AND year = ?')
...
export function saveCompanyFinanceMonthNotes(month: number, year: number, notes: string): FinanceMonthInsight {
  const db  = getDb()
  const now = Date.now()
  db.prepare(`
    INSERT INTO company_finance_month_insights (id, month, year, notes, ai_analysis, ai_generated_at, created_at, updated_at)
    VALUES (?, ?, ?, ?, NULL, NULL, ?, ?)
  ...
  return getCompanyFinanceMonthInsight(month, year) as FinanceMonthInsight
}
 ...
 */
export function saveCompanyFinanceMonthAIAnalysis(month: number, year: number, analysis: string): FinanceMonthInsight {
  const db  = getDb()
  const now = Date.now()
  db.prepare(`
    INSERT INTO company_finance_month_insights (id, month, year, notes, ai_analysis, ai_generated_at, created_at, updated_at)
...
export function getCompanyFinanceCategoryBreakdown(month: number, year: number): FinanceCategoryBreakdownItem[] {
  const movements   = listCompanyFinanceMovements(month, year)
  const totalActual = sumActual(movements)
 ...
 */
export function getCompanyFinanceHistory(month: number, year: number, monthsBack: number): FinanceHistoryEntry[] {
  ...
  for (let i = monthsBack - 1; i >= 0; i--) {
    ...
    const movements = listCompanyFinanceMovements(m, y)
...
export function getCompanyFinanceTopConcepts(month: number, year: number, limit = 8): FinanceRankingConcept[] {
  const movements   = listCompanyFinanceMovements(month, year)
  const totalActual = sumActual(movements)
 ...
 */
export function getCompanyFinanceTopIncreases(month: number, year: number, limit = 8): FinanceRankingIncrease[] {
  const movements = listCompanyFinanceMovements(month, year)
 ...
 */
export function buildCompanyFinanceImportPreview(
  ...
  const existingRows = db.prepare(
    'SELECT id, concept_id FROM company_finance_movements WHERE month = ? AND year = ?'
 ...
 */
export function confirmCompanyFinanceImport(
  items: FinanceImportConfirmItem[], month: number, year: number
): FinanceImportResult {
  const db  = getDb()
  const now = Date.now()
  const conceptsById = new Map(listCompanyFinanceConcepts().map(c => [c.id, c]))
